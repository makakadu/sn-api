...
class PostParamsValidator extends \App\Application\Common\PostParamsValidator {
    ...
    /** @param mixed $value */
    static function validateParamHideSignatureIfEdited($value) {
        try {
            Assertion::boolean($value, "Param 'hide_signature_if_edited' should be a boolean");
        } catch (\Assert\InvalidArgumentException $ex) {
            throw new MalformedRequestException($ex->getMessage());
        }
    }
    ...
    /** @param mixed $value */
    static function validateParamAddSignature($value) {
        try {
            Assertion::boolean($value, "Param 'add_signature' should be a boolean");
        } catch (\Assert\InvalidArgumentException $ex) {
            throw new MalformedRequestException($ex->getMessage());
        }
    }
